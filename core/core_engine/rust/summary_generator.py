...
"""
    Rust 函数摘要生成器
    ~~~~~~~~~~~~~~~~~~~
    用 tree-sitter 解析 Rust 源文件，提取每个函数的返回值数据流摘要。
    摘要只记录数据流事实，不做安全判定。
...
_RUST_LITERAL_IDENTS = frozenset({"true", "false"})
            ...
            if child.type == "let_declaration":
                ...
                for c in child.children:
                    ...
                    if init_expr is None and c.type in (
                        ...
                        "macro_invocation", "field_expression",
...
def _trace_dataflow(
    ...
    # 4. method_call_expression
    if expr_node.type == "method_call_expression":
    ...
    # 5. macro_invocation
    if expr_node.type == "macro_invocation":
        macro_name = "<macro>"
        for c in expr_node.children:
            if c.type == "identifier":
    ...
    if expr_node.type == "unary_expression":
        for child in expr_node.children:
            if child.type in _RUST_LITERAL_IDENTS:
                continue
    ...
    # 9. index_expression
    if expr_node.type == "index_expression":
        for child in expr_node.children:
            if child.type in ("[", "]"):
                continue
            return _trace_dataflow(
...
def generate_file_summaries(file_path: str, file_content: str) -> FileSummary:
    ...
    try:
        ...
        logger.warning(f"解析 Rust 文件失败 {file_path}: {e}")
        return FileSummary(file=file_path, content_hash=content_hash, functions=[])
    ...
    for child in root.children:
        if child.type == "function_item":
            summary = _process_func_node(child, file_path, file_lines, in_impl=False)
            if summary:
                functions.append(summary)
        elif child.type == "impl_item":
            # Walk methods inside impl
            for sub in child.children:
                if sub.type == "function_item":
                    summary = _process_func_node(sub, file_path, file_lines, in_impl=True)
                    if summary:
                        functions.append(summary)
        elif child.type == "mod_item":
            # Walk functions inside mod
            block = None
            for c in child.children:
                if c.type == "block":
                    block = c
            ...
            if block:
                for sub in block.children:
                    if sub.type == "function_item":
                        summary = _process_func_node(sub, file_path, file_lines, in_impl=False)
                        if summary:
                            functions.append(summary)
