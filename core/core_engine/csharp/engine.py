...
def init_match_rule(data):
    ...
    if isinstance(obj, str):
        ...
        # strip namespace prefix: Namespace.Class.Func → Func
        if '.' in function_name:
            function_name = function_name.split('.')[-1]
        # 匹配 Namespace.Class.Func(...) 和 Func(...)
        match = (r"(?:^|[\s=,.])\w+\." + re.escape(function_name) + r"\s*\([^)]*\)" +
                 r"|" +
                 r"(?:^|[\s=,.])" + re.escape(function_name) + r"\s*\([^)]*\)")
        # 匹配函数定义
        match2 = r"(?:public|private|protected|static|virtual|override|async|internal)\s+" + \
