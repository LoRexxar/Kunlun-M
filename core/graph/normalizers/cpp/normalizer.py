...
Converts C++ source parsed by ``tree-sitter-cpp`` (via ``tree_sitter.Parser``)
into the unified intermediate representation used by the AST graph engine.

Extends C Normalizer with C++-specific constructs:
  - class_specifier (with constructors, destructors, virtual methods)
  - template_declaration / template_parameter_list
  - namespace_definition
  - lambda_expression
  - range_based_for, try/catch
  - new/delete, throw, nullptr

tree-sitter C++ node model (same as C):
  - ``node.type`` -> str
  - ``node.text`` -> bytes
...
class Normalizer:
    ...
    def normalize(
        ...
        if source_content:
            content_hash = hashlib.md5(
                (source_content.encode("utf-8", errors="ignore") if isinstance(source_content, str) else source_content)
            ).hexdigest()
    ...
    def _walk_node(self, node, add_node, add_edge,
        ...
        if ntype == "for_range_loop":
            return self._walk_range_for(node, add_node, add_edge, ctx_stack,
                                         file_path, depth)
        ...
        if ntype == "catch_clause":
            return self._walk_catch(node, add_node, add_edge, ctx_stack,
                                     file_path, depth)

        # ---- New / Delete expression ------------------------------------
        if ntype == "new_expression":
            return self._walk_new(node, add_node, add_edge, ctx_stack,
                                   file_path, depth)
        if ntype == "delete_expression":
            return self._walk_delete(node, add_node, add_edge, ctx_stack,
                                      file_path, depth)
    ...
    def _walk_function_def(self, node, add_node, add_edge,
        ...
        # Determine function type based on context and name
        func_type = FunctionType.FUNCTION.value
        if func_decl:
            has_destructor = self._find_child_by_type(func_decl, "destructor_name")
            if has_destructor:
                func_type = FunctionType.DESTRUCTOR.value
            elif ctx_stack:
                parent_pos, parent_label = ctx_stack[-1]
                if parent_label == NodeLabel.CLASS.value:
                    if ret_type == "" or ret_type == name.lstrip("~"):
    ...
    def _walk_class(self, node, add_node, add_edge,
        ...
        lineno = self._lineno(node)
        type_id = self._find_child_by_type(node, "type_identifier")
        name = self._text(type_id) if type_id else "<anonymous_class>"

        # Check for base class (inheritance)
        base_class = ""
        for child in node.children:
            if child.type == "base_class_clause":
                base_class = child.text.decode(errors="replace").strip()
        ...
        # Traverse child nodes (methods, constructors, destructors) with class context
        ...
        for child in node.children:
            ...
            if child.type in ("field_declaration", "access_specifier",
                              "{", "}", "class", ";", ":",
                              "type_identifier", "base_class_clause"):
                continue
            self._walk_node(child, add_node, add_edge, ctx_stack,
                           file_path, child_depth)
            child_depth += 1
    ...
    def _walk_lambda(self, node, add_node, add_edge,
        ...
        capture = ""
        for child in node.children:
            if child.type == "lambda_capture":
                capture = child.text.decode(errors="replace").strip()
                break

        pos = add_node({
            "label": NodeLabel.FUNCTION.value,
            "name": f"<lambda{capture}>",
            "lineno": lineno,
            "end_lineno": end_lineno,
            "language": self.language,
            "attrs": {
                "fullname": f"<lambda{capture}>",
                "type": FunctionType.LAMBDA.value,
                "signature": f"<lambda{capture}>",
                "file_path": file_path,
                "raw_type": "LambdaExpression",
            },
        })
        self._own_edge(add_edge, ctx_stack, pos, depth)
    ...
    # ===================================================================
    # C++-specific: range-based for
    # ===================================================================

    def _walk_range_for(self, node, add_node, add_edge,
                         ctx_stack, file_path, depth) -> int:
        lineno = self._lineno(node)
        pos = add_node({
            "label": NodeLabel.BRANCH.value,
    ...
    def _walk_try(self, node, add_node, add_edge,
        ...
        for c_idx, catch in enumerate(catches):
            c_pos = self._walk_catch(catch, add_node, add_edge, ctx_stack,
                                      file_path, c_idx)
    ...
    # ===================================================================
    # C++-specific: new / delete
    # ===================================================================

    def _walk_new(self, node, add_node, add_edge,
                   ctx_stack, file_path, depth) -> int:
        lineno = self._lineno(node)
        callee = node.text.decode(errors="replace").strip()
        callee = callee[:60]
    ...
    def _walk_delete(self, node, add_node, add_edge,
                      ctx_stack, file_path, depth) -> int:
        lineno = self._lineno(node)
        pos = add_node({
            "label": NodeLabel.OPERATOR.value,
    ...
    def _walk_using(self, node, add_node, add_edge,
        ...
        qual_id = self._find_child_by_type(node, "qualified_identifier")
        name = self._text(qual_id) if qual_id else "using"
        # Or scope_resolution expression
        if not qual_id:
            name = node.text.decode(errors="replace").strip().rstrip(";").strip()
