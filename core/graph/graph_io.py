"""AST Graph I/O — 持久化 igraph 图到压缩 GraphML 文件。
...
class AstGraphIO:
    ...
    将 igraph.Graph 保存为 gzip 压缩的 GraphML（``.graphmlz``），
    ...
    def save(self, graph: "igraph.Graph") -> dict:  # type: ignore[name-defined]
        ...
        保存流程：
            1. 确保目录存在；
            2. 用 ``format='graphmlz'`` 将 graph 写入 ``graph_path``；
    ...
    def load(self) -> "igraph.Graph | None":  # type: ignore[name-defined]
        ...
        meta = self.get_meta()
        if meta is None:
            print(f"[AstGraphIO] warning: meta.json missing, skip loading {self.graph_path}")
            return None

        actual_hash = _md5_file(self.graph_path)
        expected_hash = meta.get("content_hash")
        if not expected_hash or actual_hash != expected_hash:
            print(
                f"[AstGraphIO] warning: content_hash mismatch "
                f"(expected={expected_hash}, actual={actual_hash}), "
                f"file may be corrupted: {self.graph_path}"
            )
            return None
    ...
    def exists(self) -> bool:
        """判断图文件是否存在。"""
        return os.path.isfile(self.graph_path)
